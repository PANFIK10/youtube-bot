...
WORDS_PER_CHAPTER = 480 # Идеальный расчет: 32 главы по 480 слов = 15360 слов (~118 минут)
MODEL_NAMES = {
...
async def generate_script(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        for i, chapter in enumerate(chapters):
            ...
            # 2. Жёсткие рамки и физический лимит токенов
            chapter_prompt = (
                f"Тема видео: {data['topic']}\n"
                f"Суть этой части: {chapter}\n"
                f"Стиль: {style_prompt}\n\n"
                f"КРИТИЧЕСКИЕ ПРАВИЛА (ШТРАФ ЗА НАРУШЕНИЕ):\n"
                f"1. ОБЪЕМ: Напиши СТРОГО 450-500 слов. Не пиши слишком длинно! Заверши мысль, уложившись в этот объем.\n"
                f"2. ФОРМАТ: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО писать заголовки! Никаких символов '#', никаких 'Глава', 'Часть'. Пиши ТОЛЬКО сплошной текст абзацами.\n"
