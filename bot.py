...
async def settings_menu(message: types.Message):
    ...
    # Словарь для превращения технического ID в красивое название
    friendly_names = {
    ...
    # Получаем красивое имя или оставляем ID, если вдруг модель не в списке
    display_name = friendly_names.get(current_model_id, current_model_id)
    
    info = (
        "⚙️ **Настройки интеллекта**\n\n"
        f"Текущая модель: **{display_name}**\n\n"
        "📜 **Доступные варианты:**\n"
        "• **Claude** — Идеальный стиль и живой язык.\n"
        "• **ChatGPT** — Стабильная логика и точность.\n"
        "• **Gemini** — Огромный объем памяти для длинных сценариев.\n"
        "• **Grok** — 🚀 Дерзкие и нестандартные сценарии.\n"
        "• **Qwen** — Высокая скорость и четкое следование правилам.\n"
    )
...
async def change_model(message: types.Message):
    ...
    selected_id = model_map[message.text]
